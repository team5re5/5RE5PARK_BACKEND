...
public class MemberExceptionHandler {
    ...
    // 회원 파트에서 RuntimeException이 발생한 경우
    @ExceptionHandler(RuntimeException.class)
    public ResponseEntity<ErrorResponseDto> handleRuntimeException(RuntimeException e) {
        // 로그로 어떤 예외 클래스가 발생했는지 표시
        log.error("MEMBER : [RuntimeException] ", e);
    ...
    // 회원 파트에서 BusinessException이 발생한 경우
    @ExceptionHandler(BusinessException.class)
    public ResponseEntity<ErrorResponseDto> handleBusinessException(BusinessException e) {
        // 로그로 어떤 예외 클래스가 발생했는지 표시
        log.error("MEMBER : [BusinessException] ", e);
    ...
    // 회원 파트에서 MethodArgumentNotValidException이 발생한 경우 - 데이터 유효성 검증 실패
    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseEntity<ErrorResponseDto> handleMethodArgumentNotValidException(
            MethodArgumentNotValidException e) {
        // 로그로 어떤 예외 클래스가 발생했는지 표시
        log.error("MEMBER : [MethodArgumentNotValidException] ", e);
        ...
        ErrorResponseDto response =
                ErrorResponseDto.of(
                        MEMBER_INVALID_INPUT_VALUE_ERROR.getStatus(),
                        MEMBER_INVALID_INPUT_VALUE_ERROR.getMessage(),
                        fieldErrorList);
    ...
    // 회원 파트에서 ConstraintViolationException 발생한 경우 - 데이터베이스 제약 조건 위반할 경우 발생(JPA/Hibernate)
    @ExceptionHandler(ConstraintViolationException.class)
    public ResponseEntity<ErrorResponseDto> handleConstraintViolationException(
            ConstraintViolationException e) {
        // 로그로 어떤 예외 클래스가 발생했는지 표시
        log.error("MEMBER : [ConstraintViolationException] ", e);
        ...
        // 응답 데이터 반환
        return ResponseEntity.status(MEMBER_INVALID_INPUT_VALUE_ERROR.getStatus()).body(response);
    }
}
